...
- **⚠️ เตือนทันทีในข้อความแรกของ session ถัดไป (นับจาก 2026-09-19):** มีปัญหาค้างเรื่อง iPad status bar ที่ยังไม่แก้ — ดูรายละเอียดที่ข้อ 11 ในหัวข้อ "จุดเทคนิคที่ต้องระวัง" ด้านล่าง ผู้ใช้ขอให้เตือนเรื่องนี้ทันทีที่เริ่มคุยกันครั้งถัดไป (ลบ bullet นี้ทิ้งได้หลังจากเตือนแล้ว/คุยเรื่องนี้จบแล้ว)
...
11. **ปัญหายังไม่แก้ (2026-09-19): iPad โชว์ status bar (เวลา/แบต) ค้างอยู่แม้ Add to Home Screen แล้ว** — ทดสอบแล้วว่า Safari "Add to Home Screen" (standalone web app) และ Guided Access **ไม่สามารถซ่อน status bar ได้เลย** เป็นข้อจำกัดของ iOS เอง ไม่เกี่ยวกับ HTTPS/root cert ทางแก้ที่หาไว้คือต้องใช้แอป **kiosk browser จาก App Store** (เช่น "Kiosker: Fullscreen Web Kiosk") แทนการเปิดผ่าน Safari ตรงๆ — ยังไม่ได้ทดลองจริง รอทำตอนเราเตอร์ TP-Link มาถึงพร้อมกับขั้นตอน mkcert root cert (ดู README/บทสนทนาก่อนหน้า)
